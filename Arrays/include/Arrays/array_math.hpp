    ...
    for (uint32_t i = 0; i < m; ++i) {
        for (uint32_t k = 0; k < n; ++k) {
            for (uint32_t j = 0; j < p; ++j) {
                (*result.data_ptr)[i * p + j] += (*t1.data_ptr)[i * n + k] * (*t2.data_ptr)[k * p + j];
            }
        }
    }
    return result;
...
T vector_mult(const Array<T>& t1, const Array<T>& t2){
    ...
    for (uint32_t i = 0; i < t1.data_ptr->size(); ++i) {
        sum += (*t1.data_ptr)[i] * (*t2.data_ptr)[i];
    }
    return sum;
}
template<typename T>
Array<T> matrix_vector_mult(const Array<T>& t1, const Array<T>& t2){
    ...
    for (uint32_t i = 0; i < m; ++i) {
        for (uint32_t j = 0; j < n; ++j) {
            (*result.data_ptr)[i] += (*t1.data_ptr)[i * n + j] * (*t2.data_ptr)[j];
        }
    }
    return result;
}
...
Array<T> vector_matrix_mult(const Array<T>& t1, const Array<T>& t2){
    ...
    for (uint32_t j = 0; j < m; ++j) {
        for (uint32_t k = 0; k < n; ++k) {
            (*result.data_ptr)[k] += (*t1.data_ptr)[j] * (*t2.data_ptr)[j * n + k];
        }
    }
    return result;
}
