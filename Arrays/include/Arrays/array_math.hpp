...
namespace nd {
    ...
    Array<T> matrix_mult(const Array<T>& t1, const Array<T>& t2){
        ...
        for (uint32_t bi = 0; bi < m; bi += BLOCK_SIZE) {
            for (uint32_t bk = 0; bk < n; bk += BLOCK_SIZE) {
                for (uint32_t bj = 0; bj < p; bj += BLOCK_SIZE) {
                    // inside loop
                    uint32_t i_end = bi + BLOCK_SIZE < m ? bi + BLOCK_SIZE : m;
                    uint32_t k_end = bk + BLOCK_SIZE < n ? bk + BLOCK_SIZE : n;
                    uint32_t j_end = bj + BLOCK_SIZE < p ? bj + BLOCK_SIZE : p;

                    for (uint32_t i = bi; i < i_end; ++i) {
                        for (uint32_t k = bk; k < k_end; ++k) {
                            T val1 = (*t1.data_ptr)[i * t1.strides[0] + k * t1.strides[1] + t1.offset];
                            for (uint32_t j = bj; j < j_end; ++j) {

                                (*result.data_ptr)[i * p + j] += val1 * (*t2.data_ptr)[k * t2.strides[0] + j * t2.strides[1] + t2.offset];
                            }
                        }
                    }
                }
